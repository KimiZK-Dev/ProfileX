document.addEventListener("DOMContentLoaded", () => {
	...
	const musicPlayer = {
		playYouTubeAudio(url) {
			...
			fetch(
					...
					const playPromise = new Promise((resolve, reject) => {
						audio
							...
							.catch((error) => {
								...
								reject(
									"Không thể phát audio: " + error.message
								);
							});
				...
				.catch((error) => {
					console.error(
						"Lỗi tổng thể khi xử lý link YouTube:",
						error
					);
					FuiToast.error(
						`Có lỗi khi xử lý link YouTube: ${error.message}`
					);
				});
		},
	...
	const lockScreen = {
		init() {
			...
			const title =
				$("html").attr("data-title-loader") || "Màn Hình Khóa";
			$("body").append(`
	...
	const greetingManager = {
		...
		getGreeting() {
			...
			if (hour >= 3 && hour <= 10) selectedGreetings = greetings.morning;
				...
				selectedGreetings = greetings.noon;
	...
	const descriptionManager = {
		...
		updateDescription() {
			...
			this.element.fadeOut(500, () =>
				this.element.html(text).fadeIn(500)
			);
		},
			...
			if (
				...
				Fancybox.bind("[data-fancybox]", {
					...
					on: {
						...
						closed: (fancybox) =>
							fancybox.$container?.removeAttribute("inert"),
					},
				});
			} else {
				console.error(
					"Fancybox not loaded or no [data-fancybox] found"
				);
			}
		},
	};
