document.addEventListener("DOMContentLoaded", () => {
	...
	const randomUtils = {
		generateMathString() {
			...
			return `${num1 < 10 ? "0" + num1 : num1}.${num2 < 10 ? "0" + num2 : num2}.${
	...
	const toastManager = {
		init() {
			if (cookieManager.getCookie("toast") === "close") $("#toast-prompt").hide();
	...
	const musicPlayer = {
		playYouTubeAudio(url) {
			console.log("Bắt đầu xử lý link YouTube (API mới):", url);

			// Gọi API mới để lấy thông tin video và media
			fetch(`https://api.zm.io.vn/v1/social/autolink?url=${encodeURIComponent(url)}`, {
				headers: {
				...
				.then((data) => {
					console.log("Dữ liệu từ API zm.io.vn:", data);

					// Hiển thị thông tin video (ví dụ: title, thumbnail, duration)
					FuiToast.success(`Đã lấy thông tin video: ${data.title}`);
					// Render thumbnail, title, duration ra UI nếu muốn
					// Ví dụ: $("#video-title").text(data.title); $("#video-thumb").attr("src", data.thumbnail);

					// Tìm media audio (ưu tiên mp3, hoặc type: 'audio')
					const audioMedia = data.medias.find((m) => m.type === "audio" || m.type === "Audio");
					if (!audioMedia) throw new Error("Không tìm thấy audio trong danh sách media!");
					...
					audio
						...
						.then(() => {
							...
							FuiToast.success("Đang phát audio!");
						})
						.catch((e) => {
							console.error("Không thể phát audio:", e);
							FuiToast.error("Không thể phát audio: " + e.message);
						});
				})
				.catch((error) => {
					console.error("Lỗi khi xử lý link YouTube:", error);
                ...
                <section class="td-welcome">
                    ...
                    <div class="infos">
                        <div class="logo-web-title">
                            <img class="logo-ws" src="./assets/img/icon/iconLockScreen.jpg" alt="Vương Thanh Diệu">
	...
	const scrollToTop = {
		init() {
			...
			window.addEventListener("scroll", () => {
				...
				const percentage = totalHeight > 0 ? Math.round((scrollY / totalHeight) * 100) : 0;
	...
	const starAnimation = {
		animateStar(star) {
			star.style.setProperty("--star-left", `${randomUtils.randomRange(-10, 100)}%`);
			star.style.setProperty("--star-top", `${randomUtils.randomRange(-40, 80)}%`);
	...
	const clickEffect = {
		...
		init() {
			let messageIndex = 0;
			$("body").click((event) => {
				const text = $(`<span style='font-family:sans-serif;'>`).text(this.messages[messageIndex]);
				...
				$("body").append(text);
				text.animate({ top: event.pageY - 180, opacity: 0 }, 1500, () => text.remove());
			});
		},
	};
	...
	const quoteManager = {
		fetchQuote() {
			$.ajax({
				...
				type: "get",
				dataType: "json",
				success: (data) => $("#cham-ngon").fadeOut(300).text(data.msg).fadeIn(300),
	...
	const greetingManager = {
		...
		getGreeting() {
			...
			else if (hour >= 16 && hour <= 18) selectedGreetings = greetings.afternoon;
	...
	const descriptionManager = {
		...
		updateDescription() {
			...
			const text = this.descriptions[Math.floor(Math.random() * this.descriptions.length)];
		...
		init() {
			...
			if (!this.element.length) {
				console.error("Không tìm thấy phần tử .web_desc.text-center trong DOM khi khởi tạo!");
				return;
			}
		...
		init() {
			...
			const loadingInterval = setInterval(() => {
				...
				if (progress.length) {
					...
					progress.css("transform", `translate3d(${parseInt(percentage)}%, 0px, 0px)`);
					if (percentage === "100%") {
						$(".pace-active").animate({ top: "-100px" }, "slow", function () {
							$(this).hide();
						});
						$("#loading-box").is(":visible") ? this.endLoading() : $(".td-loading-v2").fadeOut(1200);
	...
	class Sakura {
		...
		draw(ctx) {
			...
			ctx.drawImage(sakuraAnimation.image, 0, 0, 30 * this.scale, 30 * this.scale);
			ctx.restore();
		}
	...
	const sakuraAnimation = {
		...
		initCanvas() {
			...
			this.canvas.style.cssText = "position: fixed; left: 0; top: 0; pointer-events: none; z-index: 8888;";
		...
		init() {
			if (typeof Fancybox !== "undefined" && document.querySelector("[data-fancybox]")) {
				Fancybox.bind("[data-fancybox]", {
					...
					autoFocus: false,
					on: {
						init: (fancybox) => fancybox.$container?.removeAttribute("aria-hidden"),
						closing: (fancybox) => {
							const trigger = fancybox.$trigger || document.querySelector("[data-fancybox]:focus");
